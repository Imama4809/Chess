...
def is_this_a_possible_move(dict, chosen_piece, chosen_move, col):
    ...
    do_move((check_dict[chosen_piece].position[0]-1)*50,(8-check_dict[chosen_piece].position[1])*50,(int(interchange_number_and_letter(chosen_move[0]))-1)*50,(8-int(chosen_move[1]))*50,check_dict)
    if is_king_in_path_of_enemy_pieces(col_king,col,check_dict) == True:
        settings.turn = og_turn
        return False
    settings.turn = og_turn
    return True 

def possible_moves_when_in_check(dict,col):
    possible_move_list = []
    col_list = []
    for val in dict:
    ...
    for chosen_piece in col_list:
        for chosen_move in dict[piece].move_list():
            check_dict = copy.deepcopy(dict)
            do_move((check_dict[chosen_piece].position[0]-1)*50,(8-check_dict[chosen_piece].position[1])*50,(int(interchange_number_and_letter(chosen_move[0]))-1)*50,(8-int(chosen_move[1]))*50,check_dict)
            if is_king_in_path_of_enemy_pieces(col_king,col,check_dict) == False:
                settings.turn = og_turn
                break
            settings.turn = og_turn
